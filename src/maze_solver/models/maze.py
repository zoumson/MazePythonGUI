...
def validate_entrance(maze: Maze) -> None:
    assert sum(1 for square in maze if square.role is Role.ENTRANCE) == 1, "Must be exactly one entrance"


def validate_exit(maze: Maze) -> None:
    assert sum(1 for square in maze if square.role is Role.EXIT) == 1, "Must be exactly one exit"
